/*
Чтобы добавить новый тип заказа (заказ с предоплатой), нужно:
...
4. Создать PrepaidOrderProcessing : OrderProcessing, где:

        Pay(order): принять предоплату → order.MarkPaid().

        Доставка: либо переопределить Deliver(order) и не отгружать без оплаты, либо добавить в базовый класс хук protected virtual bool CanShip(Order) и в PrepaidOrderProcessing вернуть order.IsPaid.

ProcessOrder() не меняем: предоплата реализуется переопределением шагов в подклассе.
 * */


using System;

namespace T4TemplateMethodPattern
{
