...
/*
Для нового вида расчёта (например, скидка) просто создаём новый класс-посетитель, 
наследуемый от OrderVisitor — например, DiscountCalculator, и реализуем в нём VisitProduct() и VisitBox().

Код классов Product, Box и Order менять не нужно, так как у них уже есть метод Accept(visitor).

Нужно вносить изменения только в том случае, если появляется новый тип элемента структуры.
*/
namespace T8VisitorPattern
{
    ...
    class Program
    {
        ...
        {
            var laptop = new Product("Ноутбук", 120000m);
            ...
            var usbFlash = new Product("USB-флешка", 1200m);
            var extendedWarranty = new Product("Дополнительная гарантия", 5000m);
            ...
            // используем посетителей для расчёта доставки и налогов
