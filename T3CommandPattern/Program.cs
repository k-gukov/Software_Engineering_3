...
//Чтобы отменить несколько действий, команды последовательно извлекаются из стека (LIFO) и для каждой вызывается метод Undo().
//Таким образом, система откатывает состояние шаг за шагом в обратном порядке. То есть у нас будет повторная отмена в цикле.

//Ограничения:
//1. Не все команды обратимы — если действие не изменило состояние лифта, оно не попадёт в историю и не может быть отменено.
//2. История команд может расти без ограничений.
//3. Состояние лифта должно изменяться только через команды, иначе Undo станет некорректным.

//Зато при моей текущей реализации команда поездки на этаж отменяется правильно, целиком (не происходит снижение лифта на один этаж)


using System;
using System.Collections.Generic;

namespace T3CommandPattern
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            while (true)
            {
                Console.WriteLine();
                Console.WriteLine(
                    $"Состояние: этаж {elevator.CurrentFloor}, двери {(elevator.IsDoorOpen ? "открыты" : "закрыты")}");
    ...
    public struct Floor
    {
        ...
        {
            Console.WriteLine("На этаже {0}: {1}, {2}", Number, RoomA, RoomB);
        }
    }
    ...
    public class Elevator
    {
        ...
        private Elevator(int bottomFloor, int topFloor)
        {
            ...
            Console.WriteLine("Лифт установлен на {0} этаже", CurrentFloor);
        }
    ...
    public abstract class Command
    {
        ...
        // флаг, чтобы не добавлять в историю неуспешные команды
    ...
    public class GoToFloorCommand : Command //команда движения на определённый этаж
    {
        ...
        public override void Execute()
        {
            ...
            // двигаемся к определённому этажу (вверх или вниз)
