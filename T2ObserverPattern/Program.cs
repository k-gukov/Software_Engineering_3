...
namespace T2ObserverPattern
{
    ...
    public class AnalyticsSystem : IOrderObserver
    {
        ...
        {
            Console.WriteLine(
                $"[Аналитика] Зафиксировано изменение статуса заказа #{order.Id}: {previousStatus.ToRussian()} -> {order.Status.ToRussian()}.");
        }
    }
    ...
    internal class Program
    {
        ...
        {
            //создаём заказ и наблюдателей
            var order = new Order(id: 101, initialStatus: OrderStatus.Placed);

            var clientNotification = new ClientNotification("Константин Гуков", "kgukov@example.com");
            var managerNotification = new ManagerNotification("Мария Смирнова");
            ...
            Console.WriteLine("Изменяем статус заказа...\n");
